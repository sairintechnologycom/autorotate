...
import type { VarRecord, RiskItem, PatternMatch, Severity } from './types.js';
...
export function classifyVar(v: VarRecord): RiskItem {
  ...
  const runbook = matches.length > 0 ? RUNBOOK[matches[0].runbookId] : undefined;
  
  return { variable: v, matches, severity, rationale: buildRationale(v, matches), runbook };
}
